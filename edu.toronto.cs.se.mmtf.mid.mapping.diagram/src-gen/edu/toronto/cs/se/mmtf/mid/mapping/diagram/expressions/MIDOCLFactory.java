...
public class MIDOCLFactory {
	...
	protected MIDOCLFactory() {
		this.expressions = new MIDAbstractExpression[1];
	}
	...
	public static MIDAbstractExpression getExpression(int index,
		...
		MIDOCLFactory cached = MIDDiagramEditorPlugin.getInstance()
				.getMIDOCLFactory();
		if (cached == null) {
			MIDDiagramEditorPlugin.getInstance().setMIDOCLFactory(
					cached = new MIDOCLFactory());
		}
		if (index < 0 || index >= cached.expressions.length) {
			throw new IllegalArgumentException();
		}
		if (cached.expressions[index] == null) {
			final String[] exprBodies = new String[] { "not self.oclIsTypeOf(mapping::BinaryMapping)", //$NON-NLS-1$
			};
