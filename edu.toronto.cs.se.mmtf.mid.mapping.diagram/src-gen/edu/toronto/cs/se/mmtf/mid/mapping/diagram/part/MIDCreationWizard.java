...
public class MIDCreationWizard extends Wizard implements INewWizard {
	...
	public void init(IWorkbench workbench, IStructuredSelection selection) {
		...
		setDefaultPageImageDescriptor(MIDDiagramEditorPlugin
				.getBundledImageDescriptor("icons/wizban/NewMappingWizard.gif")); //$NON-NLS-1$
	...
	public void addPages() {
		diagramModelFilePage = new MIDCreationWizardPage(
				"DiagramModelFile", getSelection(), "mapping_diagram"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		domainModelFilePage = new MIDCreationWizardPage(
				"DomainModelFile", getSelection(), "mapping") { //$NON-NLS-1$ //$NON-NLS-2$

			public void setVisible(boolean visible) {
				if (visible) {
					String fileName = diagramModelFilePage.getFileName();
					fileName = fileName.substring(0, fileName.length()
							- ".mapping_diagram".length()); //$NON-NLS-1$
					setFileName(MIDDiagramEditorUtil.getUniqueFileName(
							getContainerFullPath(), fileName, "mapping")); //$NON-NLS-1$
				}
