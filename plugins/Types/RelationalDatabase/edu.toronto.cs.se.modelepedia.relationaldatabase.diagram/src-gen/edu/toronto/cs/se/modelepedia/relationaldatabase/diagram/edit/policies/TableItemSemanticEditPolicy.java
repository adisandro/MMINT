...
public class TableItemSemanticEditPolicy extends
	...
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (RelationalDatabaseVisualIDRegistry.getVisualID(node)) {
			case TableTableColumnsCompartmentEditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit
						.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (RelationalDatabaseVisualIDRegistry
							.getVisualID(cnode)) {
					case ColumnEditPart.VISUAL_ID:
						for (Iterator<?> it = cnode.getTargetEdges().iterator(); it
								.hasNext();) {
							Edge incomingLink = (Edge) it.next();
							if (RelationalDatabaseVisualIDRegistry
									.getVisualID(incomingLink) == ForeignKeyTableColumnEditPart.VISUAL_ID) {
							...
							if (RelationalDatabaseVisualIDRegistry
									.getVisualID(incomingLink) == ForeignKeyForeignColumnEditPart.VISUAL_ID) {
								DestroyReferenceRequest r = new DestroyReferenceRequest(
										incomingLink.getSource().getElement(),
										null, incomingLink.getTarget()
												.getElement(), false);
				...
				for (Iterator<?> cit = node.getChildren().iterator(); cit
					...
					case ForeignKeyEditPart.VISUAL_ID:
						for (Iterator<?> it = cnode.getSourceEdges().iterator(); it
							...
							if (RelationalDatabaseVisualIDRegistry
									.getVisualID(outgoingLink) == ForeignKeyTableColumnEditPart.VISUAL_ID) {
								DestroyReferenceRequest r = new DestroyReferenceRequest(
							...
							if (RelationalDatabaseVisualIDRegistry
									.getVisualID(outgoingLink) == ForeignKeyForeignColumnEditPart.VISUAL_ID) {
								DestroyReferenceRequest r = new DestroyReferenceRequest(
										outgoingLink.getSource().getElement(),
										null, outgoingLink.getTarget()
												.getElement(), false);
