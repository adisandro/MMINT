...
public class ColumnItemSemanticEditPolicy extends
	...
	public ColumnItemSemanticEditPolicy() {
		super(RelationalDatabaseElementTypes.Column_3001);
	}
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (RelationalDatabaseVisualIDRegistry.getVisualID(incomingLink) == ForeignKeyTableColumnEditPart.VISUAL_ID) {
			...
			if (RelationalDatabaseVisualIDRegistry.getVisualID(incomingLink) == ForeignKeyForeignColumnEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(
						incomingLink.getSource().getElement(), null,
						incomingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));
