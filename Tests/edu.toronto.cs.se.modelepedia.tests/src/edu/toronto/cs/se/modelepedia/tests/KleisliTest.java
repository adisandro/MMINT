...
public class KleisliTest {
	...
	private final static String[] SRC_METAMODELOBJ_NAMES = {"Bank"};
	private final static String[] TGT_METAMODELOBJ_NAMES = {"Company"};
	...
	public void test() throws Exception {
		...
		String kSrcMetamodelName = SRC_MODELTYPEENDPOINT_NAME + MMTF.ENDPOINT_SEPARATOR + SRC_MODELTYPE_NAME + MMTF.MODEL_FILEEXTENSION_SEPARATOR + EcorePackage.eNAME;
		...
		KleisliModelEndpointReference kSrcModelTypeEndpointRef = (KleisliModelEndpointReference) kModelRelType.getModelEndpointRefs().get(0);
		EPackage srcMetamodelRootObj = (EPackage) MultiModelUtils.getModelFileInState(kSrcMetamodelName);
		ModelEndpointReference tgtModelTypeEndpointRef = kModelRelType.getModelEndpointRefs().get(1);
		EPackage tgtMetamodelRootObj = (EPackage) MultiModelUtils.getModelFileInState(tgtMetamodelName);
		ModelElement rootModelElemType = rootModelType.getModelElems().get(0);
		Link rootLinkType = ((ModelRel) kRootModelRelType.getSupertype()).getLinks().get(0);
		for (int i = 0; i < SRC_METAMODELOBJ_NAMES.length; i++) {
			EClass metamodelObj = (EClass) srcMetamodelRootObj.getEClassifier(SRC_METAMODELOBJ_NAMES[i]);
			if (!kSrcModelTypeEndpointRef.acceptModelElementType(metamodelObj)) {
				continue;
			}
			...
			ModelElementReference newModelElemTypeRef = rootModelElemType.createSubtypeAndReference(null, modelElemTypeUri, newModelElemTypeName, eInfo, kSrcModelTypeEndpointRef);
			metamodelObj = (EClass) tgtMetamodelRootObj.getEClassifier(TGT_METAMODELOBJ_NAMES[i]);
			if (!tgtModelTypeEndpointRef.acceptModelElementType(metamodelObj)) {
				continue;
			}
			modelElemTypeUri = MultiModelRegistry.getModelAndModelElementUris(metamodelObj, MidLevel.TYPES)[1];
			eInfo = MultiModelRegistry.getModelElementEMFInfo(metamodelObj, MidLevel.TYPES);
			newModelElemTypeName = MultiModelRegistry.getModelElementName(eInfo, metamodelObj, MidLevel.TYPES);
			newModelElemTypeRef = rootModelElemType.createSubtypeAndReference(null, modelElemTypeUri, newModelElemTypeName, eInfo, kSrcModelTypeEndpointRef);
			//TODO[TESTS] finish this
			rootLinkType.createSubtypeAndReference(null, "", true, kModelRelType);
		}
		//TODO MMTF[TESTS] do I need to invoke MMTF.createTypeHierarchy(multiModel)?
		...
		Model bankModel = srcModelType.createInstanceAndEditor(TESTS_BUNDLE_TEST_DIR + IPath.SEPARATOR + SRC_MODELTYPE_NAME.toLowerCase() + MMTF.MODEL_FILEEXTENSION_SEPARATOR + MultiModelTypeFactory.ECORE_REFLECTIVE_FILE_EXTENSION, ModelOrigin.CREATED, instanceMID);
