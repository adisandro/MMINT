...
public class OpenTypeMidHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		String midFileName = path + IPath.SEPARATOR + MMTF.TYPE_MID_FILENAME;
		String middiagFileName = midFileName + DIAGRAM_SUFFIX;
		File middiag = new File(middiagFileName);
		if (!middiag.exists()) {
			generateDiagram(midFileName);
		}
	...
	private void generateDiagram(String domainModelFileName) {
		...
		try {
			String diagramModelFileName = domainModelFileName + DIAGRAM_SUFFIX;

			// load the domain model resource
			ResourceSet domainResourceSet = new ResourceSetImpl();
			URI domainFileURI = URI.createFileURI(domainModelFileName);
			...
			URI diagramFileURI = URI.createFileURI(diagramModelFileName);
			...
			MMTFException.print(MMTFException.Type.WARNING, "Error saving Type MID diagram", e);
		}
	}
}
