...
/**
 * The semantic edit policy for imported model relationships.
 * 
 * @author Alessio Di Sandro
 * 
 */
public class ModelRelImportedSemanticEditPolicy extends ModelRel2ItemSemanticEditPolicy {

	/**
	 * Gets the command to destroy a model relationship.
	 * 
	 * @param req
	 *            The request.
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (MidVisualIDRegistry.getVisualID(incomingLink) == ModelParameterEditPart.VISUAL_ID) {
				DestroyElementRequest r = new DestroyElementRequest(
						incomingLink.getElement(), false);
				cmd.add(new DestroyElementCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), incomingLink));
			...
			if (MidVisualIDRegistry.getVisualID(incomingLink) == ModelParameter2EditPart.VISUAL_ID) {
				DestroyElementRequest r = new DestroyElementRequest(
						incomingLink.getElement(), false);
				cmd.add(new DestroyElementCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), incomingLink));
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (MidVisualIDRegistry.getVisualID(outgoingLink) == ModelRelModelsEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(
						outgoingLink.getSource().getElement(), null,
						outgoingLink.getTarget().getElement(), false);
				cmd.add(new ModelRelRemoveModelCommand(r));
	 ...
	 * originating from a model relationship.
	 * 
	 * @param req
	 *            The request.
	 * @return The executable command.
