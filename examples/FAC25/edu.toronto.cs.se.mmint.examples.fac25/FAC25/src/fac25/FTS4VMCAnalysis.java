...
public class FTS4VMCAnalysis implements IPLGSNAnalysis {
  ...
  protected String productLine2Dot(ProductLine productLine, String name) {
    ...
    for (var s : productLine.getClasses()) {
      if (s.instanceOf(types.getInitialState())) {
        out += "\n  " + s.getAttribute(types.getAbstractState_Name()) + "[initial=True]";
        break;
      }
    }
    for (var t : productLine.getClasses()) {
      ...
      for (var src : t.getReference(types.getTransition_Source())) {
        var srcName = src.getAttribute(types.getAbstractState_Name());
        for (var tgt : t.getReference(types.getTransition_Target())) {
          var tgtName = tgt.getAttribute(types.getAbstractState_Name());
  ...
  public void import_(PLGSNAnalyticTemplate plTemplate, ProductLine productLine) throws Exception {
    ...
    var desc = mcStrategy.getAttribute(this.gsn.getArgumentElement_Description())
      .replace("model checking", "lifted model checking");
    mcStrategy.setAttribute(this.gsn.getArgumentElement_Description(), desc);
    var liftedGoal = builder.createGoal(Map.of(
      this.gsn.getArgumentElement_Id(), "G4",
      this.gsn.getArgumentElement_Description(), "The lifted model checker is correct",
      this.gsn.getArgumentElement_TemplateId(), "liftedGoal"));
    builder.support(mcStrategy, liftedGoal);
    ...
    if (!templateElems.containsKey("safetyGoal")) {
      ...
      if (!otherTemplate.isEmpty()) {
        ...
        if (otherFilesCtx != null) { // connected with model-based template, extract model from it
          ...
          dialogInitial = safetyGoal.getAttribute(this.gsn.getArgumentElement_Description()).split("'")[1];
        }
      }
    }
    if (modelPath == null) {
  ...
  public void impact(PLGSNAnalyticTemplate plTemplate, PLGSNChangeStep step) throws Exception {
    ...
    var satSolution = templateElems.get("satSolution");
    var satGoal = templateElems.get("satGoal");
    // obsolete branch, do not re-run model checking but mark to revise
    if (prevImpact.get(ImpactType.REVISE).isPresent() && prevImpact.get(ImpactType.REUSE).isEmpty() &&
        prevImpact.get(ImpactType.RECHECK).isEmpty()) {
      satSolution.setImpact(ImpactType.REVISE);
      satGoal.setImpact(ImpactType.REVISE);
    }
    // regression analysis: re-run model checking
      ...
      var oldHolds = satGoal.getAttribute(this.gsn.getArgumentElement_Description()).contains("holds");
    ...
    // baseline
      ...
      var prevImpactReuse = prevImpact.get(ImpactType.REUSE);
      String restorePC = null;
      if (prevImpactReuse.isPresent() && prevImpactReuse.get().equals(satSolution.getPresenceCondition())) {
        // temporarily modify presence condition to run baseline with the proper formula
        restorePC = satSolution.getPresenceCondition();
        satSolution.setPresenceCondition(prevImpactReuse.get());
      }
      var templateStep = new PLGSNChangeStep(satSolution);
      templateStep.getBackwardTrace().add(List.of());
      templateStep.baselineImpact();
      var impactTypes = satSolution.getImpact();
      satGoal.setImpact(impactTypes);
      safetyGoal.setImpact(impactTypes);
      if (restorePC != null) {
        satSolution.setPresenceCondition(restorePC);
      }
    }
  ...
  public void repair(PLGSNAnalyticTemplate plTemplate, PLGSNChangeStep step) throws Exception {
    ...
    var satImpact = satSolution.getImpact();
    ...
    var oldSatDesc = satSolution.getAttribute(this.gsn.getArgumentElement_Description());
